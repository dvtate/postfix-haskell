 ...
 * == operator
 */
class EqExpr extends DataExpr {
    /**
     * @param {Token} token - locaiton in code
     * @param {Value} lhs - left hand side
     * @param {Value} rhs - right hand side
     */
     ...
     */
    out() {
        // TODO handle tuples and shit
        return `(EQIV? ${this.lhs.out()} ${this.rhs.out})`;
    }
};
...
class FunExportExpr extends Expr {
    ...
    out() {
        ...
        })\n(export "${this.name.slice(1)}" ${this.name})`;
    }
};
