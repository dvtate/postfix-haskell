...
const operators = {
    ...
    'target' : {
        action: (ctx, token) => {
            ...
            if (sym.type !== value.ValueType.Id)
                return ['expected a symbol'];
            const act = ctx.stack.pop();
            if (act.type !== value.ValueType.Macro)
                return ['expected macro'];
            const args = ctx.stack.pop();
            if (args.type !== value.ValueType.Macro)
                return ['expected macro'];

            // Get input types
            const stackCpy = ctx.stack.slice();
            const ev = args.value.action(ctx, token);
            if (typeof ev === 'object' && !(ev instanceof Context))
                return ev;

            // Get input types
            const inTypes = ctx.stack.splice(ctx.cmpStack(stackCpy));
            if (inTypes.some(t => t.type !== value.ValueType.Type))
                return ['expected a macro of types'];

            // Put opaque param exprs onto stack
            const out = new expr.FunExportExpr(token, sym.value, inTypes);
            const pes = inTypes.map((t, i) => new expr.ParamExpr(token, t.value, out, i));
            ctx.stack.push(...pes);

            // Invoke macro to determine structure of fxn
            // TODO make sure stack is empty so that it only pulls from params
            const ev1 = act.value.action(ctx, token);
            if (typeof ev1 === 'object' && !(ev1 instanceof Context))
                return ev1;

            // Get output values
            const ovs = ctx.stack.splice(ctx.cmpStack(stackCpy));
...
const numberCheck = new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
    ...
    const ret = synTypes.includes(a.type) && synTypes.includes(b.type)
        ...
        && (a.type === value.ValueType.Expr || a.value.type === b.value.type);
...
const funs = {
    ...
    '+' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true))
                ...
                ctx.stack.push(new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.add`, [a, b]));
            else if (!exprs[0])
                // A is const, try to optimize
                // adding zero is identity
                ctx.stack.push(a.value.value == 0
                    ? b
                    : new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.add`, [a, b]));
            else // if (!exprs[1])
                // B is const, try to optimize
                // Adding zero is identity
                ctx.stack.push(b.value.value == 0
                    ? a
                    : new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.add`, [a, b]));
        })),
    )),
    '*' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true))
                ...
                ctx.stack.push(new expr.InstrExpr(token, b.datatype, `${type}.mul`, [a, b]));
            } else if (!exprs[0])
                // A is const, try to optimize
                // mul by 1 is identity
                // mul by 0 is 0
                ctx.stack.push(a.value.value == 1
                    ? b
                    : a.value.value == 0
                        ? a
                ...
                // mul by 1 is identity
                // mul by 0 is 0
                ctx.stack.push(b.value.value == 1
                    ? a
                    : b.value.value == 0
                        ? b
                        : new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.mul`, [a, b]));
    ...
    '%' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true)) {
                ...
                const type = a.datatype.getBaseType().name;
                ctx.stack.push(new expr.InstrExpr(token, b.datatype, `${type}.rem${type[0] === 'f' ? '' : '_s'}`, [a, b]));
            }
        })),
    )),
    '-' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true))
                ...
                // Neither is a constexpr
                ctx.stack.push(new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.sub`, [a, b]));
    ...
    '/' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true)) {
                ...
                // Neither is a constexpr
                const type = a.datatype.getBaseType().name;
                ctx.stack.push(new expr.InstrExpr(token, b.datatype, `${type}.div${type[0] === 'f' ? "" : "_s"}`, [a, b]));
            } else { // if (!exprs[1] && exprs[0])
                // B is const, try to optimize
    ...
    '@' : new value.Value(null, value.ValueType.Fxn, new Fun(
        null,
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => ctx.stack.pop() || ['missing argument'])),
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => ctx.invoke(ctx.stack.pop(), token))),
    )),
    ...
    '==' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            const b = ctx.stack.pop();
            const a = ctx.stack.pop();
            const isData = ![a, b].some(v => ![value.ValueType.Expr, value.ValueType.Data].includes(v));
            ...
            switch (a.type) {
                ...
                case value.ValueType.Data:
                    ...
                    if (b.type === value.ValueType.Data) {
                        ctx.stack.push(toBool(a.value.equals(b.value), token));
                        return;
                    }

                    // Runtime check
                    if (b.type === value.ValueType.Expr) {
                        // TODO inline this, remove eqexpr type
                        ctx.stack.push(new expr.EqExpr(token, a, b));
                        return;
                    }
