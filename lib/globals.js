...
const operators = {
    ...
    '=' : {
        action: ctx => {
            ...
            // Set local
            ctx.scopes[ctx.scopes.length - 1][id] = ctx.stack.pop();
        },
    },
    ...
    '+' : {
        action: ctx => {
            ...
            if (![ValueType.Macro, ValueType.Number].includes(a.type))
                return ['first value is wrong type'];
            if (![ValueType.Macro, ValueType.Number].includes(b.type))
            ...
            switch (a.type) {
                ...
                // + on macros (concat)
                case ValueType.Macro:
                    ctx.stack.push(a.value.body.concat(b.value.body));
                    return;
