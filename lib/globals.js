...
const operators = {
    ...
    'target' : {
        action: (ctx, token) => {
            ...
            const inputs = ctx.stack.slice(ctx.cmpStack(stackCpy));
    ...
    '<' : new value.Value(null, value.ValueType.Fxn, new Fun(
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true))
                ...
                ctx.push(new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.lt`, [a, b]));
        })),
    )),
    '>' : new value.Value(null, value.ValueType.Fxn, new Fun(
        null,
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            if (!exprs.includes(true))
                ...
                // Result unknown at compile time (probably)
                ctx.push(new expr.InstrExpr(token, b.datatype, `${a.datatype.getBaseType().name}.gt`, [a, b]));
        })),
    )),
    // TODO comparisons: < >
    // TODO type-casting
