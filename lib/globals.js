...
const operators = {
    ...
    'target' : {
        action: (ctx, token) => {
            ...
            // Get input types
            // TODO replace with ctx.traceIO()
            const inTypes = ctx.stack.slice(ctx.cmpStack(stackCpy));
            ctx.popn(inTypes.length);
            ...
            // Get output values
            // TODO replace with ctx.traceIO()
            const ovs = ctx.stack.slice(ctx.cmpStack(stackCpy));
            ctx.popn(ovs.length);
...
const funs = {
    ...
    '@' : new value.Value(null, value.ValueType.Fxn, new Fun(
        null,
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => ctx.pop() || ['missing argument'])),
        ...
        new value.Value(null, value.ValueType.Macro, new Macro((ctx, token) => {
            ...
            switch (a.type) {
                ...
                case value.ValueType.Expr:
                    ...
                    ctx.push(new expr.EqExpr(token, a, b));
                    return;
