    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        switch (t.type) {
            ...
            case lex.TokenType.Block:
                const scopesCp = scopes.slice();
                ctx.stack.push(new value.Value(t, value.ValueType.Macro, new Macro((ctx, token) => {
                    // Invoke macro in new scope
                    // TODO on error include macro token
                    // TODO clone context and use old scopes
                    const ret = parse(t.body, ctx, scopesCp);
                    ctx.scopes.pop();
                    scopesCp.pop();
                    return ret;
                }, ctx, t.body)));
            ...
            case lex.TokenType.Identifier:
                ...
                if (t.subtype == 'escaped') {
                    ...
                    ctx.stack.push(new value.IdValue(t, { id: t.token, scope: ctx.scopes.slice() }));
                } else if (t.subtype == 'upper') {
                    // TypeName
                    ctx.stack.push(new value.Value(t, value.ValueType.TypeName, { id: t.token, scope: ctx.scopes.slice() }));
                } else {
                    // Invoke operator
