    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        switch (t.type) {
            ...
            case lex.TokenType.Number:
                ctx.stack.push(new Value(ValueType.Number, t.value, t));
                break;

            // Blocks: Need to form a closure with current scope
            case lex.TokenType.Block:
                ctx.stack.push(new Value(ValueType.Macro, new Macro(parse.bind(null, t.body), ctx, t.body), t));
            ...
            case lex.TokenType.Identifier:
                ...
                if (t.subtype == 'escaped') {
                    ...
                    ctx.stack.push(new Value(ValueType.Id, { id: t.token, scope: ctx.scopes.slice() }, ));
                } else if (t.subtype == 'upper') {
                    // TypeName
                    ctx.stack.push(new Value(ValueType.TypeName, { id: t.token, scope: ctx.scopes.slice() }, t));
                } else {
                    // Invoke operator
