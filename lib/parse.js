    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        switch (t.type) {
            ...
            case lex.TokenType.Block:
                ctx.stack.push(new value.Value(t, value.ValueType.Macro, Macro.fromLiteral(ctx, t, parse)));
                break;
            ...
            case lex.TokenType.Identifier:
                ...
                if (t.token[0] === '$') {
                    ...
                    // Convert Functions to Macros to make life easier
                    // TODO should just make invoke method instead
                    if (v.type === value.ValueType.Fxn)
                    ...
                    if (v.type === value.ValueType.Macro) {
                        ...
                        if (ret instanceof Array)
                            return new error.SyntaxError(ret.map(e => `${t.token}: ${e}`).join('; '), t, ctx);
